...
function clienteDadosVazios(): array {
    return ['documento'=>'','razao_social'=>'','nome_fantasia'=>'','telefone'=>'','email'=>'','cep'=>'','logradouro'=>'','numero'=>'','complemento'=>'','bairro'=>'','cidade'=>'','estado'=>'','latitude'=>'','longitude'=>'','observacoes'=>'','ativo'=>true];
}
function clienteDadosPost(): array {
    $t=static fn(string $c):string=>trim((string)($_POST[$c]??''));
    return ['documento'=>preg_replace('/\\D+/','',$t('documento'))??'','razao_social'=>$t('razao_social'),'nome_fantasia'=>$t('nome_fantasia'),'telefone'=>$t('telefone'),'email'=>mb_strtolower($t('email')),'cep'=>preg_replace('/\\D+/','',$t('cep'))??'','logradouro'=>$t('logradouro'),'numero'=>$t('numero'),'complemento'=>$t('complemento'),'bairro'=>$t('bairro'),'cidade'=>$t('cidade'),'estado'=>mb_strtoupper($t('estado')),'latitude'=>str_replace(',','.',$t('latitude')),'longitude'=>str_replace(',','.',$t('longitude')),'observacoes'=>$t('observacoes'),'ativo'=>isset($_POST['ativo'])];
}
function clienteDocumentoValido(string $d):bool {
...
function clienteErros(array $d):array {
    $e=[];if(!clienteDocumentoValido($d['documento']))$e[]='Informe um CNPJ válido.';
